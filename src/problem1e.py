...
from problem1d import (Access, AuthorizedTechnicalSupport, Client,
                       ComplianceOfficer, DefaultRole, FinancialAdvisor,
                       FinancialPlanner, InvestmentAnalyst, PremiumClient,
                       Resource, TechnicalSupport, Teller, get_role_from_str)
...
class TestAccessControl(unittest.TestCase):
    ...
    def test_client_role_permissions(self):
        ...
        self.assertEqual(self.client_role.get_permissions(), expected_permissions)

    def test_premium_client_role_permissions(self):
        expected_permissions = {
            Resource.ACCOUNT_BALANCE: Access.VIEW,
        ...
        self.assertEqual(self.premium_client_role.get_permissions(), expected_permissions)

    def test_financial_planner_role_permissions(self):
        expected_permissions = {
            Resource.ACCOUNT_BALANCE: Access.NO,
            Resource.INVESTMENT_PORTFOLIO: Access.VIEW,
            Resource.FINANCIAL_ADVISOR_CONTACT_DETAILS: Access.NO,
        ...
        self.assertEqual(self.financial_planner_role.get_permissions(), expected_permissions)

    def test_financial_advisor_role_permissions(self):
        expected_permissions = {
            Resource.ACCOUNT_BALANCE: Access.NO,
            Resource.INVESTMENT_PORTFOLIO: Access.VIEW,
            Resource.FINANCIAL_ADVISOR_CONTACT_DETAILS: Access.VIEW,
        ...
        self.assertEqual(self.financial_advisor_role.get_permissions(), expected_permissions)

    def test_investment_analyst_role_permissions(self):
        expected_permissions = {
            Resource.ACCOUNT_BALANCE: Access.NO,
            Resource.INVESTMENT_PORTFOLIO: Access.VIEW,
            Resource.FINANCIAL_ADVISOR_CONTACT_DETAILS: Access.NO,
        ...
        self.assertEqual(self.investment_analyst_role.get_permissions(), expected_permissions)

    def test_technical_support_role_permissions(self):
        expected_permissions = {
            Resource.ACCOUNT_BALANCE: Access.NO,
            Resource.INVESTMENT_PORTFOLIO: Access.NO,
            Resource.FINANCIAL_ADVISOR_CONTACT_DETAILS: Access.NO,
        ...
        self.assertEqual(self.technical_support_role.get_permissions(), expected_permissions)

    def test_authorized_technical_support_role_permissions(self):
        expected_permissions = {
            Resource.ACCOUNT_BALANCE: Access.NO,
            Resource.INVESTMENT_PORTFOLIO: Access.NO,
            Resource.FINANCIAL_ADVISOR_CONTACT_DETAILS: Access.NO,
        ...
        self.assertEqual(self.authorized_technical_support_role.get_permissions(), expected_permissions)

    def test_compliance_officer_role_permissions(self):
        expected_permissions = {
            Resource.ACCOUNT_BALANCE: Access.VIEW,
            Resource.INVESTMENT_PORTFOLIO: Access.VIEW,
            Resource.FINANCIAL_ADVISOR_CONTACT_DETAILS: Access.VIEW,
        ...
        self.assertEqual(self.compliance_officer_role.get_permissions(), expected_permissions)
    ...
    @mock.patch("problem1d.datetime")
    @mock.patch("problem1d.datetime")
    def test_financial_advisor_role_permissions_during_working_hours(self, mock_datetime):
        mock_datetime.utcnow.return_value.time.return_value = time(10, 0)
        expected_permissions = {
            Resource.FINANCIAL_ADVISOR_SYSTEM_ACCESS: Access.VIEW,
            # Add other resources and their expected access values during working hours
        }
        self.assertEqual(self.financial_advisor_role.get_permissions(), expected_permissions)
...
if __name__ == "__main__":
    unittest.main()
    @mock.patch("problem1d.datetime")
    def test_financial_advisor_role_permissions_outside_working_hours(self, mock_datetime):
        mock_datetime.utcnow.return_value.time.return_value = time(18, 0)
        expected_permissions = {
            Resource.FINANCIAL_ADVISOR_SYSTEM_ACCESS: Access.NO,
            # Add other resources and their expected access values outside working hours
        }
        self.assertEqual(self.financial_advisor_role.get_permissions(), expected_permissions)
